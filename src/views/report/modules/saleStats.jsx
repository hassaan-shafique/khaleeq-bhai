...
const SaleStats = ({ id, salesData, installments }) => {
    ...
    const getSalesData = () => {
      ...
      salesData.forEach(sale => {
        if (sale.startDate && sale.startDate.seconds) {
          ...
          if (withinCustomRange) {
            console.log({ sale, start, end, saleDate })
          ...
          <Table stickyHeader>
            ...
            <TableBody>
              ...
              {saleStats.map((sale, index) => (
                <SaleDataStats sale={sale} installments={installments} index={index} />
              ))}
            </TableBody>
          </Table>
        </TableContainer>
            ...
            {loading ? (
              ...
              saleStats.map((sale, index) => {
                ...
                const latestInstallmentDate = saleInstallments.length
                  ? new Date(Math.max(...saleInstallments.map(inst => new Date(inst.date)))).toLocaleDateString('en-GB') // This formats as "dd/mm/yyyy"
                  : 'No Date'
                ...
                return sale.totalAmount ? (
                  ...
                  >
                    ...
                    <TableCell>
                      <Typography variant='body2'>{formatTimestamp(sale.startDate)}</Typography>
                    </TableCell>
                    <TableCell>
        ...
        {pieData.length > 0 && (
          <Typography variant='h6' color='black' sx={{ marginBottom: 2 }}>
            ...
            <Typography variant='h6'>
              <strong>{pieData.reduce((max, entry) => (entry.value > max.value ? entry : max)).name}</strong> with Rs{' '}
              {pieData.reduce((max, entry) => (entry.value > max.value ? entry : max)).value}
            </Typography>
          </Typography>
        )}
